...
def main():
    ...
    print("\n INITIAL SHAPE:", series_dict['20220224'].shape,"\n")
                
    if args.type is not None:
        output_path = os.path.join(args.out_base, args.type + "_mining.pickle")
    else:
        output_path = os.path.join(args.out_base, str(dt.now().strftime("%m/%d/%Y_%H:%M:%S")) + "_mining.pickle")
    ...
    output_ids = thread_series(series_corrected)
